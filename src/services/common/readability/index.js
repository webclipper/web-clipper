...
let readability = {
  ...
  init: function () {
    ...
    let nextPageLink = readability.findNextPageLink(document.body);
    ...
    if (readability.frameHack) {
      let readOverlay = document.getElementById('readOverlay');
    ...
    **/
    if ((window.location.protocol + '//' + window.location.host + '/') === window.location.href) {
      ...
      let rootWarning = document.createElement('p');
    ...
    document.onkeydown = function (e) {
      let code = (window.event) ? event.keyCode : e.keyCode;
      if (code === 16) {
      ...
      if (code === 32) {
        ...
        let windowHeight = window.innerHeight ? window.innerHeight : (document.documentElement.clientHeight ? document.documentElement.clientHeight : document.body.clientHeight);
  ...
  **/
  ...
  getSuggestedDirection: function (text) {
    ...
    function countMatches(match) {
      let matches = text.match(new RegExp(match, 'g'));
      return matches !== null ? matches.length : 0;
    }

    function isRTL() {
      let count_heb = countMatches('[\\u05B0-\\u05F4\\uFB1D-\\uFBF4]');
      let count_arb = countMatches('[\\u060C-\\u06FE\\uFB50-\\uFEFC]');
  ...
  prepDocument: function () {
    ...
    if (frames.length > 0) {
      let bestFrame = null;
      let bestFrameSize = 0;    /* The frame to try to run readability upon. Must be on same domain. */
      let biggestFrameSize = 0; /* Used for the error message. Can be on any domain. */
    ...
    for (let k = 0; k < document.styleSheets.length; k += 1) {
  ...
  **/
  addFootnotes: function (articleContent) {
    let footnotesWrapper = document.getElementById('readability-footnotes');

    let articleFootnotes = document.getElementById('readability-footnotes-list');
    ...
    if (!footnotesWrapper) {
      ...
      let readFooter = document.getElementById('readFooter');
    ...
    let articleLinks = articleContent.getElementsByTagName('a');
    let linkCount = articleFootnotes.getElementsByTagName('li').length;
    for (let i = 0; i < articleLinks.length; i += 1) {
      let articleLink = articleLinks[i];

      let footnoteLink = articleLink.cloneNode(true);

      let refLink = document.createElement('a');

      let footnote = document.createElement('li');

      let linkDomain = footnoteLink.host ? footnoteLink.host : document.location.host;

      let linkText = readability.getInnerText(articleLink);
  ...
  **/
  grabArticle: function (page) {
    let stripUnlikelyCandidates = readability.flagIsActive(readability.FLAG_STRIP_UNLIKELYS);
    ...
    **/
    ...
    for (let pt = 0; pt < nodesToScore.length; pt += 1) {
      ...
      let innerText = readability.getInnerText(nodesToScore[pt]);
    ...
    for (let s = 0, sl = siblingNodes.length; s < sl; s += 1) {
      ...
      **/
      ...
      if (siblingNode.nodeName === 'P') {
        let linkDensity = readability.getLinkDensity(siblingNode);
        let nodeContent = readability.getInnerText(siblingNode);
  ...
  **/
  getLinkDensity: function (e) {
    ...
    let textLength = readability.getInnerText(e).length;
    let linkLength = 0;
  ...
  **/
  findBaseUrl: function () {
    let noUrlParams = window.location.pathname.split('?')[0];
  ...
  **/
  findNextPageLink: function (elem) {
    ...
    let articleBaseUrl = readability.findBaseUrl();
    ...
    **/
    for (let i = 0, il = allLinks.length; i < il; i += 1) {
      ...
      let linkText = readability.getInnerText(link);
      ...
      readability.ajax(pageUrl, {
        success: function (r) {
          ...
          **/
          ...
          let nextPageLink = readability.findNextPageLink(page);

          let content = readability.grabArticle(page);
          ...
          **/
          ...
          if (firstP && firstP.innerHTML.length > 100) {
            for (let i = 1; i <= readability.curPageNum; i += 1) {
              let rPage = document.getElementById('readability-page-' + i);
  ...
  cleanConditionally: function (e, tag) {
    ...
    **/
    for (let i = curTagsLength - 1; i >= 0; i -= 1) {
      let weight = readability.getClassWeight(tagsList[i]);
      let contentScore = (typeof tagsList[i].readability !== 'undefined') ? tagsList[i].readability.contentScore : 0;
      ...
      if (weight + contentScore < 0) {
        ...
        **/
        ...
        let linkDensity = readability.getLinkDensity(tagsList[i]);
        let contentLength = readability.getInnerText(tagsList[i]).length;
        let toRemove = false;
  ...
  **/
  scrollTop: function (scroll) {
    let setScroll = typeof scroll !== 'undefined';
  ...
  **/
  ...
  scrollTo: function (scrollStart, scrollEnd, steps, interval) {
    if (
      ...
      let oldScrollTop = readability.scrollTop();
