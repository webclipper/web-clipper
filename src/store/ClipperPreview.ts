...
export class ClipperScreenshootPreviewDate implements ClipperPreiviewData {
  ...
  constructor(selectArea: SelectAreaPosion, base64Capture: string, baseHost: string) {
  ...
  perpare = async () => {
    ...
    const result = await axios.post(`${this.baseHost}/api/upload/attach?ctoken=${cookie}&type=image`, formData, {
      headers: {
